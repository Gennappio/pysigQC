...
#
# Produces three types of plots per signature:
#   1. NA proportion barchart: fraction of samples with NA values per signature gene
#   2. Expression barchart: fraction of samples with expression above threshold per gene
#   3. Density plots: smoothed distributions of the above proportions
#
# Expression threshold: user-provided per dataset, or defaults to median of all genes
# across all samples in each dataset.
#
# Computes 2 radar chart metrics:
#   - med_prop_na: median proportion of non-NA values across signature genes (1 - NA rate)
#   - med_prop_above_med: median proportion of samples above threshold across signature genes
#
# NOTE: Genes missing from a dataset are handled differently for NA vs expression:
#   - For NA analysis: missing genes are set to 1 (all NA) — line 57
#   - For expression analysis: missing genes are set to 0 (none expressed) — line 74
# This asymmetry is intentional but may be confusing without documentation.
