...
#
# Evaluates the compactness (internal coherence) of gene signatures by computing
# Spearman autocorrelation matrices between all pairs of signature genes.
#
# Three main outputs:
#   1. Autocorrelation heatmaps: Spearman correlation matrix of signature genes,
#      one heatmap per dataset-signature combination (ComplexHeatmap)
#   2. Density plots: distribution of pairwise autocorrelation values
#   3. Rank Product analysis (optional, requires RankProd package + >1 dataset):
#      identifies genes that are consistently poorly correlated with other signature
#      genes across multiple datasets, suggesting they may not belong in the signature
#
# The median autocorrelation value is stored as the 'autocor_median' radar chart metric.
#
# BUG: Missing genes are padded with min(expression) values (same issue as eval_struct_loc),
# which introduces artificial correlation patterns.
# ISSUE: Single-gene signatures produce a 1x1 correlation matrix; edge case handling may be incorrect.
