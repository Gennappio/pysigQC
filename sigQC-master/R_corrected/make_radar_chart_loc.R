...
#
# Creates the final summary radar chart aggregating all 14 QC metrics computed by
# the preceding pipeline steps. Each signature-dataset combination produces one polygon
# on the radar chart.
#
# The 14 radar chart metrics (all scaled to [0,1]):
#   1. sd_median_ratio       — Relative median SD of signature vs all genes
#   2. abs_skewness_ratio    — Absolute skewness ratio of signature vs all genes
#   3. prop_top_10_percent   — Proportion of signature genes in top 10% CV
#   4. prop_top_25_percent   — Proportion of signature genes in top 25% CV
#   5. prop_top_50_percent   — Proportion of signature genes in top 50% CV
#   6. coeff_of_var_ratio    — Coefficient of variation ratio
#   7. med_prop_na           — Median proportion of non-NA values across genes
#   8. med_prop_above_med    — Median proportion of samples above expression threshold
#   9. autocor_median        — Median intra-signature Spearman autocorrelation
#  10. rho_mean_med          — Spearman rho between mean and median scores
#  11. rho_pca1_med          — Spearman rho between PCA1 and median scores
#  12. rho_mean_pca1         — Spearman rho between mean and PCA1 scores
#  13. prop_pca1_var         — Proportion of variance explained by PC1
#  14. standardization_comp  — Spearman rho between raw median and z-median
#
# ISSUE: Line 44 applies abs() to the entire matrix, converting negative correlations
# to positive. This loses directionality — e.g., negative autocorrelation (anti-correlation
# between signature genes) is informative but is shown as positive on the chart.
#
# ISSUE: The "area ratio" computation (line 106) uses sum(r_i * r_{i+1}) / n_metrics.
# The correct polygon area formula is 0.5 * sum(r_i * r_{i+1} * sin(2*pi/n)).
# The missing sin() factor means the computed value is a dimensionless proxy, not the
# actual area ratio. Results are still ordinal (larger polygon = higher proxy) but the
# absolute values are not meaningful areas.
