...
# For each gene in the signature, applies per-gene z-transformation:
#   z_gene = (expr - mean(expr)) / sd(expr)   across samples
# Then computes per-sample median of z-transformed values ("z-median") and
# correlates it (Spearman) with the per-sample median of raw expression ("median").
# High correlation means standardization has little effect on sample ranking;
# low correlation suggests the signature is sensitive to expression scale differences.
#
# Output: scatter plot (median vs z-median) with density overlay, Spearman rho displayed.
...
eval_stan_loc <- function(gene_sigs_list, names_sigs,mRNA_expr_matrix, names_datasets,out_dir = '~',file=NULL,showResults = FALSE,radar_plot_values ){
  ...
  for(k in 1:length(names_sigs)){ 
    ...
    for (i in 1:length(names_datasets)){
      ...
      for (gene in inter){
          # BUG (division by zero): If a gene has zero variance (constant expression
        # across all samples), sd() returns 0 and division produces NaN/Inf.
        # These propagate silently through na.omit() downstream, changing the
        # effective signature size without warning.
        # FIX: Guard with if(sd == 0) set z-scores to 0.
