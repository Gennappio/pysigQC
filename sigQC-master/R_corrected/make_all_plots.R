...
make_all_plots <- function(gene_sigs_list, mRNA_expr_matrix,names_sigs=NULL,names_datasets=NULL , covariates=NULL, thresholds=NULL, out_dir = tempdir(), showResults = TRUE,origin=NULL, doNegativeControl=TRUE, numResampling=50){
  # ===========================================================================================
  # MAIN ENTRY POINT — orchestrates the entire sigQC pipeline
  # ===========================================================================================
  # Pipeline execution order:
  #   1. Input validation (signatures, datasets, names, origin)
  #   2. Output directory setup (with Windows long-path workaround)
  #   3. Remove invalid signatures (<2 genes) and datasets (<2 samples or <2 shared genes)
  #   4. eval_var_loc()         → Variability metrics (SD, CV, skewness)
  #   5. eval_expr_loc()        → Expression level metrics (NA proportion, threshold)
  #   6. eval_compactness_loc() → Autocorrelation + rank products
  #   7. compare_metrics_loc()  → Mean/Median/PCA1/GSVA/ssGSEA/PLAGE + mixture models + QQ
  #   8. eval_stan_loc()        → Standardization comparison (raw vs z-transformed)
  #   9. eval_struct_loc()      → Hierarchical clustering + biclustering
  #  10. make_radar_chart_loc() → Summary radar chart from accumulated metrics
 ...
 radar_plot_values <- list();
  ...
  tryCatch({
    ...
    # Negative controls create deeply nested directories. On Windows 7/8/10 with
    # path length >100 chars, this can exceed the 260-char MAX_PATH limit.
    # Workaround: map out_dir to a virtual drive letter (e.g., Y:) using `subst`.
    ...
    if ((length(names_datasets)== length(mRNA_expr_matrix)) && (length(names_sigs)==length(gene_sigs_list))){
     ...
     # utils::write.table('',file=file.path(out_dir, "log.log"))
      ...
      # --- Validation: remove signatures with fewer than 2 genes ---
  ...
  #        stop("Every signature must contain at least 2 elements.")
      ...
      if (length(sigs_to_remove_ind) > 0){
        # BUG (critical): Uses `k` (the last loop variable value = length(names_sigs))
        # instead of iterating over sigs_to_remove_ind. This only removes the LAST
        # signature, not all flagged signatures.
        # FIX: Should be gene_sigs_list[names_sigs[sigs_to_remove_ind]] <- NULL
        gene_sigs_list[names_sigs[sigs_to_remove_ind]] <- NULL
