...
#
# DUPLICATED CODE: This file contains plot-free versions of all eval_*_loc functions.
# It exists solely to support the negative/permutation control pipeline
# (.sigsQcNegativeControl), which needs to compute QC metrics without generating PDFs.
#
# Each *_noplots function below is a near-exact copy of its plotting counterpart,
# with all graphics calls (pdf(), par(), plot(), etc.) removed. This means:
#   - Any bug in an eval_*_loc function must be fixed in TWO places
#   - The plotting and non-plotting versions can silently diverge
#   - ~40% of the total codebase is duplication
#
# REFACTORING OPPORTUNITY: Split each eval_*_loc into compute_*() (pure computation)
# + plot_*() (visualization). The negative control pipeline would call compute_*()
# directly, eliminating this entire file.
#
# Parent functions (in their respective files):
#   eval_var_loc_noplots     → eval_var_loc.R:eval_var_loc()
#   eval_expr_loc_noplots    → eval_expr_loc.R:eval_expr_loc()
#   eval_compactness_loc_noplots → eval_compactness_loc.R:eval_compactness_loc()
#   compare_metrics_loc_noplots  → compare_metrics_loc.R:compare_metrics_loc()
#   eval_stan_loc_noplots    → eval_stan_loc.R:eval_stan_loc()
#   make_radar_chart_loc_noplots → make_radar_chart_loc.R:make_radar_chart_loc()
#
...
compare_metrics_loc_noplots <- function(gene_sigs_list,names_sigs, mRNA_expr_matrix, names_datasets, out_dir = '~',file=NULL,showResults = FALSE,radar_plot_values){
  ...
  for(k in 1:length(names_sigs)){
    ...
    for (i in 1:length(names_datasets)){
      ...
      tryCatch({
        # BUG-6: Fix na.omit(t(...)) to t(na.omit(...))
        pca1_scores <- stats::prcomp(t(stats::na.omit(data.matrix[inter,])),retx=T) #compute PCA1
