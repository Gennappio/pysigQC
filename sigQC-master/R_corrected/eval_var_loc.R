# eval_var_loc.R
#
# Evaluates expression variability of signature genes relative to all genes in the dataset.
#
# Produces mean vs SD scatter plots (signature genes highlighted) and computes:
#   - sd_median_ratio: ratio of median SD of signature genes to median SD of all genes
#   - abs_skewness_ratio: |skewness(signature)| / |skewness(all genes)|
#   - prop_top_10/25/50_percent: proportion of signature genes in the top 10/25/50% of
#     coefficient of variation across all genes (higher = more variable signature)
#   - coeff_of_var_ratio: ratio of median CV of signature genes to median CV of all genes
#
# These 6 metrics (of the 14 total) feed into the radar chart.
# Quantile lines at 10%, 25%, 50%, 75%, 90% of all-gene CV are drawn on the scatter plot.
# Per-gene mean/SD tables are written to out_dir for each signature-dataset combination.
